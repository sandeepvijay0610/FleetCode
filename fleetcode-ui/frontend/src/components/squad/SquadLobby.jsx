...
import { FleetCodeAPI } from '../../services/api.js'; // <-- UNCOMMENTED
...
export default function SquadLobby({ username, onSquadSelected }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If successful, pass the squad name up to App.jsx to unlock the Dashboard!
      onSquadSelected(inputValue);

    } catch (err) {
      // Safely grab the error message our Node.js server sent back
      setErrorMsg(err.response?.data?.error || 'Transmission failed. Try again.');
    } finally {
      setIsLoading(false);
    }
  };
